# FEP — AMBER Thermodynamic Integration workflow
...
└── LCN_to_ACN/                         # generated by fep_runner.py setup
    ...
    │   ├── EQUILIBRATION.cmd          # serial/parallel mode (min+heating+equil)
...
simulation:
  min:
    maxcyc: 1000000          # maximum minimisation steps
    ntpr: 5000               # print frequency

  heating:
    nstlim: 1000000          # steps (1 ns at dt=0.001)
    dt: 0.001
    ntwx: 10000
    tempi: 10.0              # initial temperature (K)
...
is never double-booked. Each `run_local.sh` runs all three pre-production stages
on GPU: minimisation → NVT heating (temperature ramp 10 K → 300 K over 80 % of
1 ns, then held constant) → NPT equilibration → all production windows in the
order: mid → mid-1 → … → 1 → mid+1 → … → N.
...
# (optional) edit EQUILIBRATION.cmd / FEP_PROD_*.cmd

# Submit
python fep_runner.py submit --mode serial
```

A single `EQUILIBRATION.cmd` job runs minimisation, NVT heating, and NPT
equilibration sequentially on GPU, then submits the central production window.
The production chain continues: mid → mid-1 → … → 1 → mid+1 → … → N.
Replicas are chained end-to-end (replica 2 starts when replica 1's last window
finishes).
...
`EQUILIBRATION.cmd` runs all three pre-production stages on GPU, then submits
every replica's central window simultaneously. Within each replica the windows
fan out bidirectionally:
```
1 ← 2 ← 3 ← 4 ← 5 → 6 → 7 → 8 → 9
```
...
The suite runs `setup` in a temporary directory for all three modes and validates:

- GL lambda values for all 9 windows
- All four alchemical masks (`timask1/2`, `scmask1/2`) against config values
- Simulation parameters (`nstlim`, `dt`, `ntwe`, `ntwx`)
- `run_local.sh` generated, executable, references all windows, and uses GPU only
- `heating.in` generated for both legs with correct NVT parameters and `&wt` temperature ramp
- `run_local.sh` contains NVT heating step and reads equilibration from `heating.rst7`
- `EQUILIBRATION.cmd` generated in serial and parallel modes, GPU-only, contains all three
  stage inputs (`min.in`, `heating.in`, `equil.in`), and chains to production
- Minimisation `maxcyc` and `ntpr` match config values
...
outputs, reading the system name and legs from `config.yaml`. Covers
`heating.in`, all heating output files, and `EQUILIBRATION.cmd`.
